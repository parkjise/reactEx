...
const examples = {
  ...
  reactState: withExplanation('초기값이 빈 배열이나 null일 때, 또는 복잡한 객체/배열 상태를 업데이트할 때는 제네릭을 사용해 상태의 모양을 알려줘야 합니다.', [
    ...
    '}',
    '',
    '// --- 1. 지역 상태 (Local State) 다루기 ---',
    'export const StateExample = () => {',
    '  // 1-1. 원시 타입: 제네릭 생략 (자동 추론됨)',
    '  const [isOpen, setIsOpen] = useState(false);',
    '',
    '  // 1-2. 빈 배열, null 초기값: 제네릭 필수',
    '  const [todos, setTodos] = useState<Todo[]>([]); // 제네릭 안 쓰면 never[] 됨',
    ...
    '  });',
    '',
    '  // 중첩된 객체 업데이트 시 기존 상태(prev)를 전개연산자(...)로 복사해야 합니다.',
    '  const toggleTheme = () => {',
    '    setProfile((prev) => ({',
    '      ...prev,',
    ...
    '  };',
    '',
    '  // 배열 상태 업데이트 시 기존 배열을 복사하고 추가/수정해야 합니다.',
    '  const addTodo = (task: string) => {',
    '    setTodos((prev) => [...prev, { id: Date.now(), task, completed: false }]);',
    '  };',
    '',
    '  return <div>{profile.name} - {todos.length}개의 할일</div>;',
    '};',
    '',
    '// --- 2. 전역 상태 (Global State) - Zustand 예제 ---',
    '// Zustand 스토어의 상태와 액션(함수)을 모두 포함하는 인터페이스를 정의합니다.',
    'interface AppState {',
    '  count: number;',
    '  increment: () => void;',
    '  reset: () => void;',
    '}',
    '',
    '// create 함수에 제네릭 <AppState>를 넘겨주면 내부 state 타입이 완벽히 추론됩니다.',
...
const guides: TypescriptGuide[] = [
  ...
  {
    title: '14. 상태 관리 (useState / Zustand) 타입 지정',
    description: '지역 상태(useState)의 객체/배열 업데이트 패턴과 전역 상태(Zustand)의 타입 지정 방법을 익힙니다.',
    concept: 'useState에 제네릭을 넘기면 상태의 형태를 고정할 수 있고, Zustand 스토어 생성 시 인터페이스를 넘기면 전역 상태와 액션을 완벽히 타이핑할 수 있습니다.',
    usage: '복잡한 객체 상태의 부분 업데이트, 배열에 아이템 추가/수정, 전역 상태 스토어 생성 시 사용합니다.',
    preview: 'reactState',
    exampleTitle: 'State Management (Local & Global)',
